...
class Vallox:
    ...
    def _read_message(self) -> Optional[bytes]:
        ...
        if self.serial.in_waiting < VX_MSG_LENGTH:
            return None
        
        # Read first byte
        first_byte = self.serial.read(1)
        if len(first_byte) == 0 or first_byte[0] != VX_MSG_DOMAIN:
            return None
        ...
        # Filter messages
        valid_sender = sender_byte in [VX_MSG_MAINBOARD_1, VX_MSG_THIS_PANEL, VX_MSG_PANEL_1]
        valid_receiver = receiver_byte in [VX_MSG_PANELS, VX_MSG_THIS_PANEL, VX_MSG_PANEL_1, 
                                          VX_MSG_MAINBOARD_1, VX_MSG_MAINBOARDS]
        
        if not (valid_sender and valid_receiver):
            return None
        
        # Read rest of message
        rest = self.serial.read(VX_MSG_LENGTH - 3)
        if len(rest) != VX_MSG_LENGTH - 3:
            return None
    ...
    def _decode_flags06(self, flags06: int):
        ...
        self._check_status_change(self.data['is_switch_active'], 
                                 (flags06 & VX_06_FIREPLACE_FLAG_IS_ACTIVE) != 0)
    ...
    def _decode_program(self, program: int):
        ...
        new_value = (program & VX_PROGRAM_SWITCH_TYPE) != 0
    ...
    def _send_status_req(self):
        self._request_variable(VX_VARIABLE_STATUS)
    
    def _send_io08_req(self):
        self._request_variable(VX_VARIABLE_IO_08)
    
    def _send_flags06_req(self):
        self._request_variable(VX_VARIABLE_FLAGS_06)
    
    def _send_fan_speed_req(self):
        self._request_variable(VX_VARIABLE_FAN_SPEED)
    
    def _send_default_fan_speed_req(self):
        self._request_variable(VX_VARIABLE_DEFAULT_FAN_SPEED)
    
    def _send_service_period_req(self):
        self._request_variable(VX_VARIABLE_SERVICE_PERIOD)
    
    def _send_service_counter_req(self):
        self._request_variable(VX_VARIABLE_SERVICE_COUNTER)
    
    def _send_heating_target_req(self):
        self._request_variable(VX_VARIABLE_HEATING_TARGET)
    
    def _send_rh_req(self):
        self._request_variable(VX_VARIABLE_RH1)
    
    def _send_program_req(self):
        self._request_variable(VX_VARIABLE_PROGRAM)
    ...
    def _hex_to_fan_speed(hex_val: int) -> int:
        ...
        try:
            return VX_FAN_SPEEDS.index(hex_val) + 1
        except ValueError:
            return NOT_SET
